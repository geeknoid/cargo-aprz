...
pub struct CommonArgs {
    ...
    #[arg(long, short = 'c', value_name = "PATH", default_value = "rank.toml")]
    ...
    /// Output crate information to the console. This is the default if no other output options and if check are not specified.
...
impl Common {
    ...
    pub fn report(&self, processed_crates: impl IntoIterator<Item = CrateFacts>) -> Result<()> {
                ...
                match &facts.crates_data {
                    ProviderResult::CrateNotFound(suggestions) => {
                        if suggestions.is_empty() {
                            eprintln!("  Could not find information on crate '{}'", facts.crate_spec.name());
                        } else {
                    ...
                    ProviderResult::Error(err) => {
                        eprintln!("  Could not gather information for crate '{}': {err:#}", facts.crate_spec);
                    }
        ...
        let mut reportable_crates: Vec<ReportableCrate> = if self.eval {
            analyzable_crates
                ...
                .map(|facts| {
                    ...
                    .map_err(|e| {
                        eprintln!("Warning: Could not evaluate crate '{}': {}", facts.crate_spec.name(), e);
                        e
                    })
                    .ok();
        ...
        // If --check flag is set, exit with status code 1 if any crate is not accepted
        if self.check {
