    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    match &args.command {
        AprzSubcommand::Crates(crates_args) => process_crates(crates_args).await,
        AprzSubcommand::Deps(deps_args) => process_dependencies(deps_args).await,
        AprzSubcommand::Init(init_args) => init_config(init_args),
        AprzSubcommand::Validate(validate_args) => validate_config(validate_args),
    }
}
